...
def resolver_citi(analistas=12, min_swift=4, min_cartas=2, cap_min=15):
    ...
    # Variables de decisión (número de analistas por proceso)
    x1 = LpVariable('SWIFT',    lowBound=0)   # analistas SWIFT
    x2 = LpVariable('CarCred',  lowBound=0)   # cartas de crédito
    x3 = LpVariable('Garantias',lowBound=0)   # garantías

    # Función objetivo: Min Z = 3x1 + 5x2 + 4x3
